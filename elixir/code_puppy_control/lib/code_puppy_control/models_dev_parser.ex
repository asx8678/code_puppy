defmodule CodePuppyControl.ModelsDevParser do
  ...
  # ============================================================================
  # Registry Module (GenServer for stateful operations)
  # ============================================================================
  ...
  defmodule Registry do
        ...
        {:ok, %{status: 200, body: body}} ->
          case Jason.decode(body) do
            {:ok, data} when is_map(data) and map_size(data) > 0 ->
              now = System.monotonic_time(:second)
              new_state = %{state | cached_data: data, cache_time: now}
              {:ok, data, new_state}
      ...
      Enum.reduce(filters, models, fn {capability, required}, acc ->
        Enum.filter(acc, fn model ->
          if is_boolean(required) do
            ...
            Map.get(model, String.to_atom(capability)) == required
          end
        end)
      end)
    end
    ...
    defp build_config(model, provider) do
      ...
      config = if model.cost_input, do: Map.put(config, "input_cost_per_token", model.cost_input), else: config
      config = if model.cost_output, do: Map.put(config, "output_cost_per_token", model.cost_output), else: config
      config = if model.cost_cache_read, do: Map.put(config, "cache_read_cost_per_token", model.cost_cache_read), else: config

      # Add limits
      config = if model.context_length > 0, do: Map.put(config, "max_tokens", model.context_length), else: config
      config = if model.max_output > 0, do: Map.put(config, "max_output_tokens", model.max_output), else: config
      ...
      # Add modalities
      config = if model.input_modalities != [], do: Map.put(config, "input_modalities", model.input_modalities), else: config
      config = if model.output_modalities != [], do: Map.put(config, "output_modalities", model.output_modalities), else: config

      # Add metadata
      metadata = %{}
      metadata = if model.knowledge, do: Map.put(metadata, "knowledge", model.knowledge), else: metadata
      metadata = if model.release_date, do: Map.put(metadata, "release_date", model.release_date), else: metadata
      metadata = if model.last_updated, do: Map.put(metadata, "last_updated", model.last_updated), else: metadata
  ...
  @doc "Get all providers, sorted by name."
  @spec get_providers(pid() | atom()) :: [ProviderInfo.t()]
  def get_servers(pid \\ Registry), do: Registry.get_providers(pid)
  ...
  ## Examples
      ...
      affordable = ModelsDevParser.filter_by_cost(registry, models, max_input_cost: 0.001)
  """
  @spec filter_by_cost(
          pid() | atom(),
          [ModelInfo.t()],
