defmodule CodePuppyControl.Plugins.AgentTrace.Reducer do
  ...
  @empty_usage %{input_tokens: 0, output_tokens: 0, reasoning_tokens: 0,
    cached_tokens: 0, accounting: :unknown, estimated_input: nil,
    estimated_output: nil, reconciled: false}

  @doc "Create an empty trace state."
  @spec new(String.t()) :: map()
  def new(trace_id) do
    %{trace_id: trace_id, spans: %{}, span_order: [], total_input_tokens: 0,
      total_output_tokens: 0, total_cost_usd: 0.0,
      pending_reconciliation: MapSet.new(), created_at: Schema.now_iso()}
  end
  ...
  def active_spans(state), do: state.spans |> Map.values() |> Enum.filter(&(&1.status == "running"))

  defp reduce_span_started(state, event) do
    node = event.node || %{}
    span = %{span_id: event.span_id, node_id: node[:id] || "", kind: node[:kind] || :unknown,
      name: node[:name], status: "running", parent_span_id: event.parent_span_id,
      parent_node_id: node[:parent_node_id], started_at: :erlang.system_time(:millisecond) / 1000,
      ended_at: nil, duration_ms: nil, usage: @empty_usage, error: nil, child_span_ids: []}
  ...
  defp reduce_span_ended(state, event) do
    if event.span_id && Map.has_key?(state.spans, event.span_id) do
      ...
      %{state | spans: Map.put(state.spans, event.span_id,
        %{span | status: status, ended_at: ended_at, duration_ms: duration_ms, error: error})}
    else state end
  end
  ...
  defp reduce_usage_reported(state, event) do
    if event.span_id && Map.has_key?(state.spans, event.span_id) do
      ...
      new_usage = %{span.usage | input_tokens: usage[:input_tokens] || span.usage.input_tokens,
      ...
      %{state | spans: Map.put(state.spans, event.span_id, %{span | usage: new_usage}),
  ...
  defp reduce_usage_reconciled(state, event) do
    if event.span_id && Map.has_key?(state.spans, event.span_id) do
      ...
      exact = recon[:exact] || %{}
      new_usage = %{span.usage | input_tokens: exact[:input_tokens] || span.usage.input_tokens,
        ...
        accounting: :reconciled, reconciled: true}
      drift = recon[:drift] || %{}
      %{state | spans: Map.put(state.spans, event.span_id, %{span | usage: new_usage}),
