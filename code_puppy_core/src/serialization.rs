...
pub fn serialize_session_impl(messages: &Bound<'_, PyList>) -> PyResult<Vec<u8>> {
    ...
    rmp_serde::to_vec(&msgs).map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("MessagePack serialization failed: {e}")))
}

pub fn deserialize_session_impl(py: Python<'_>, data: &[u8]) -> PyResult<Py<PyList>> {
    let msgs: Vec<Message> = rmp_serde::from_slice(data)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("MessagePack deserialization failed: {e}")))?;
...
pub fn serialize_session_incremental_impl(
    ...
    let mut all: Vec<Message> = if let Some(data) = existing_data {
        rmp_serde::from_slice(data).map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("Deserialization failed: {e}")))?
    } else { Vec::new() };
    let new: Vec<Message> = new_messages.iter().map(|o| Message::from_py(&o)).collect::<PyResult<_>>()?;
    all.extend(new);
    rmp_serde::to_vec(&all).map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("Serialization failed: {e}")))
}
...
mod tests {
    ...
    #[test] fn test_roundtrip_msgpack() {
        let msg = Message { kind: "request".into(), role: Some("user".into()), instructions: None,
            parts: vec![MessagePart { part_kind: "text".into(), content: Some("hello".into()),
                content_json: None, tool_call_id: None, tool_name: None, args: None }] };
