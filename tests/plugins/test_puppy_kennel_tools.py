...
def test_resolve_wing_shortcuts(kennel_root: Path) -> None:
    ...
    assert tools._resolve_wing("", "fast-puppy", cwd).startswith("repo:")
    assert tools._resolve_wing("repo", "fast-puppy", cwd).startswith("repo:")
    assert tools._resolve_wing("agent", "fast-puppy", cwd) == "agent:fast-puppy"
    assert tools._resolve_wing("user", "fast-puppy", cwd) == "user:default"
    assert tools._resolve_wing("custom:name", "fast-puppy", cwd) == "custom:name"
...
def test_list_wings_with_counts(kennel_root: Path) -> None:
    ...
    assert "agent:fast-puppy" not in names
    for w in out.wings:
