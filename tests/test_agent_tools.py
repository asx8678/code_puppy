...
class TestGenerateSessionHashSuffix:
    ...
    def test_hash_format(self):
        ...
        # Should be 8 hex characters (uuid4-based)
        assert len(suffix) == 8
...
class TestAutoGeneratedSessionIds:
    ...
    def test_session_id_format(self):
        ...
        # And ends with an 8-char hash (uuid4-based)
        assert len(expected_format.split("-")[-1]) == 8
    ...
    def test_session_hash_suffix_format(self):
        ...
        for _ in range(5):
            ...
            session_id = f"{agent_name}-session-{hash_suffix}"
            _validate_session_id(session_id)
            # Hash should be 8 hex chars (uuid4-based)
