...
class TestCircuitBreakerInitialization:
    ...
    async def test_initialization_default_values(self):
        ...
        assert await cb.is_closed()
        assert not await cb.is_open()
        assert not await cb.is_half_open()
...
class TestCircuitBreakerStateMethods:
    ...
    @pytest.mark.asyncio
    async def test_is_half_open_method(self):
...
class TestCircuitBreakerOpenToHalfOpenTransition:
    ...
    @pytest.mark.asyncio
    async def test_timeout_zero_allows_immediate_recovery(self):
    ...
    @pytest.mark.asyncio
    async def test_large_timeout_delays_recovery(self):
    ...
    @pytest.mark.asyncio
    async def test_multiple_get_state_calls_idempotent(self):
...
class TestCircuitBreakerForceOperations:
    ...
    @pytest.mark.asyncio
    async def test_force_open_idempotent(self):
    ...
    @pytest.mark.asyncio
    async def test_force_close_idempotent(self):
        """Test that force_close() is idempotent."""
        cb = CircuitBreaker(failure_threshold=1)
        await cb.force_open()
        await cb.force_close()
        assert await cb.is_closed()
        await cb.force_close()  # Should not raise
        assert await cb.is_closed()
...
class TestCircuitBreakerRecordMethods:
    ...
    async def test_record_success_creates_task(self):
        """Test that record_success() works correctly."""
        cb = CircuitBreaker()
    ...
    async def test_record_failure_creates_task(self):
        """Test that record_failure() works correctly."""
        cb = CircuitBreaker(failure_threshold=1)
...
class TestCircuitBreakerEdgeCases:
    ...
    async def test_lock_prevents_race_conditions(self):
        ...
        assert isinstance(await cb.get_state(), CircuitState)
...
class TestCircuitBreakerIntegrationScenarios:
    ...
    async def test_full_failure_recovery_cycle(self):
        ...
        assert result2 == "OK"
        assert await cb.is_closed()  # Back to CLOSED
