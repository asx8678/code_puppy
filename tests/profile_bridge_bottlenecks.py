...
from code_puppy._core_bridge import (
    serialize_messages_for_rust, serialize_message_for_rust,
    set_rust_enabled,
)
from code_puppy.agents.agent_code_puppy import CodePuppyAgent
from _code_puppy_core import process_messages_batch, prune_and_filter, truncation_indices


def build_history(n):
    msgs = [ModelRequest(parts=[TextPart(content="You are a helpful coding assistant. " * 10)])]
    tid = 0
    for i in range(1, n):
        p = i % 5
        if p in (0, 3):
            msgs.append(ModelRequest(parts=[TextPart(content=f"User {i}: " + "x" * (100 + i*17 % 2000))]))
        elif p == 1:
            tid += 1
            msgs.append(ModelResponse(parts=[
                TextPart(content="Let me check that."),
                ToolCallPart(tool_name="cp_read_file", args=f'{{"path":"src/m{i}.py"}}', tool_call_id=f"tc-{tid}")
            ]))
        elif p == 2:
            ...
            msgs.append(ModelResponse(parts=[TextPart(content=f"Analysis {i}: " + "detail " * (10 + i%30))]))
    return msgs[:n]
...
for N_MSGS in [50, 200, 500]:
    ...
    print(f"\n{'─'*90}")
    print(f"  {N_MSGS} messages | {total_parts} parts | {total_content_bytes:,} content bytes")
    print(f"{'─'*90}")
    ...
    call_count_truncation = 3  # processor + filter_huge + (truncation uses cached OR serializes)
    call_count_summarization = 4  # processor + filter_huge + split + prune
    call_count_finally = 1  # run_with_mcp finally block
    
    print(f"  │  Normal turn (no compaction):  {call_count_normal + call_count_finally} serializations")
    print(f"  │  Truncation compaction:         {call_count_truncation + call_count_finally} serializations")
    print(f"  │  Summarization compaction:       {call_count_summarization + call_count_finally} serializations")
    print(f"  │")
    ...
    print(f"  │  Cost for 4 redundant calls:            {statistics.median(t_ser)*4/1e6:.3f}ms")
    print(f"  └──")
    ...
    t_py_tokens = measure_ns(lambda: [agent.estimate_tokens_for_message(m) for m in msgs])
    ...
    print(f"  │    serialize_messages_for_rust:    {bridge_med/1e6:>8.3f}ms  ({bridge_med/rust_total*100:4.1f}%)")
    print(f"  │    process_messages_batch:          {rust_proc_med/1e6:>8.3f}ms  ({rust_proc_med/rust_total*100:4.1f}%)")
    print(f"  │    prune_and_filter:                {rust_prune_med/1e6:>8.3f}ms  ({rust_prune_med/rust_total*100:4.1f}%)")
    print(f"  │    truncation_indices:              {rust_trunc_med/1e6:>8.3f}ms  ({rust_trunc_med/rust_total*100:4.1f}%)")
    print(f"  │    ────────────────────────────────────────────")
    print(f"  │    TOTAL:                           {rust_total/1e6:>8.3f}ms")
    print(f"  │")
    print(f"  │  PYTHON PATH (one full turn):")
    print(f"  │    estimate_tokens × {N_MSGS}:          {py_tok_med/1e6:>8.3f}ms  ({py_tok_med/py_total*100:4.1f}%)")
    print(f"  │    hash_message × {N_MSGS}:             {py_hash_med/1e6:>8.3f}ms  ({py_hash_med/py_total*100:4.1f}%)")
    print(f"  │    prune_interrupted_tool_calls:    {py_prune_med/1e6:>8.3f}ms  ({py_prune_med/py_total*100:4.1f}%)")
    print(f"  │    truncation:                      {py_trunc_med/1e6:>8.3f}ms  ({py_trunc_med/py_total*100:4.1f}%)")
    print(f"  │    ────────────────────────────────────────────")
    print(f"  │    TOTAL:                           {py_total/1e6:>8.3f}ms")
    print(f"  │")
    speedup = py_total / rust_total if rust_total > 0 else float('inf')
    print(f"  │  Speedup (ideal, 1 serialize): {'⚡' if speedup >= 1 else '🐢'} {speedup:.2f}x")
    ...
    realistic_speedup = py_total / rust_realistic if rust_realistic > 0 else float('inf')
    print(f"  │  Speedup (realistic, 2 serializes): {'⚡' if realistic_speedup >= 1 else '🐢'} {realistic_speedup:.2f}x")
    
    # Compaction turn: 4 serializations
    rust_compaction = bridge_med * 4 + rust_proc_med + rust_prune_med * 2 + rust_trunc_med
    py_compaction = py_tok_med * 2 + py_hash_med + py_prune_med * 3 + py_trunc_med
    compaction_speedup = py_compaction / rust_compaction if rust_compaction > 0 else float('inf')
    print(f"  │  Speedup (compaction, 4 serializes): {'⚡' if compaction_speedup >= 1 else '🐢'} {compaction_speedup:.2f}x")
    print(f"  └──")
    ...
    for sz in sizes:
        ...
        print(f"  │  {sz:>6d} chars × 10 msgs: {med/1e6:.3f}ms total, {per_msg/1e6:.4f}ms/msg, {per_byte:.1f}ns/byte")
    print(f"  └──")
    ...
    t_dict_parse_plus_compute = measure_ns(lambda: process_messages_batch(serialized, [], [], ""))
    ...
    print(f"  │  truncation_indices (pure compute):        {pure_compute_med/1e6:.3f}ms")
    print(f"  │  → Dict parsing overhead estimate:         {(dict_parse_med - pure_compute_med)/1e6:.3f}ms")
    print(f"  │  → Dict parsing is {(dict_parse_med - pure_compute_med)/dict_parse_med*100:.0f}% of Rust batch call")
    print(f"  └──")
    ...
    t_getattr = measure_ns(lambda: [
        (getattr(p, "part_kind", ""), getattr(p, "content", None),
         getattr(p, "tool_call_id", None), getattr(p, "tool_name", None))
        for m in msgs for p in m.parts
    ])
    
    # Measure isinstance checks
    t_isinstance = measure_ns(lambda: [
        isinstance(getattr(p, "content", None), str) for m in msgs for p in m.parts
    ])
    
    # Measure json.dumps for non-string content
    dict_contents = [{"key": f"value_{i}", "nested": {"a": i}} for i in range(total_parts)]
    t_json = measure_ns(lambda: [json.dumps(d, sort_keys=True) for d in dict_contents[:20]])
    
    # Measure dict construction
    t_dict_build = measure_ns(lambda: [
        {"part_kind": getattr(p, "part_kind", ""), "content": getattr(p, "content", None),
         "content_json": None, "tool_call_id": getattr(p, "tool_call_id", None),
         "tool_name": getattr(p, "tool_name", None), "args": None}
        for m in msgs for p in m.parts
    ])
    ...
    print(f"  │  getattr extraction ({total_parts} parts × 4 fields): {getattr_med/1e6:.3f}ms ({getattr_med/total_bridge*100:.0f}%)")
    print(f"  │  isinstance checks:                            {isinstance_med/1e6:.3f}ms ({isinstance_med/total_bridge*100:.0f}%)")
    print(f"  │  dict construction:                             {dict_build_med/1e6:.3f}ms ({dict_build_med/total_bridge*100:.0f}%)")
    print(f"  │  remaining (json.dumps, hasattr, etc):          {(total_bridge - dict_build_med)/1e6:.3f}ms ({(total_bridge-dict_build_med)/total_bridge*100:.0f}%)")
    print(f"  └──")
    ...
    print(f"  │  Current Rust (2 serializations):        {current_rust/1e6:>8.3f}ms  ({current_py/current_rust:.2f}x)")
    print(f"  │  ✅ Cache serialized (1 serialization):   {cached_rust/1e6:>8.3f}ms  ({current_py/cached_rust:.2f}x)")
    print(f"  │  ✅ Direct pydantic access (0 bridge):    {no_bridge_rust/1e6:>8.3f}ms  ({current_py/no_bridge_rust:.2f}x)")
    print(f"  │  ✅ Optimal (no bridge + shared parse):   {optimal_rust/1e6:>8.3f}ms  ({current_py/optimal_rust:.2f}x)")
    print(f"  └──")
