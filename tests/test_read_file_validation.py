...
async def test_start_line_zero_returns_error():
...
async def test_start_line_negative_returns_error():
    with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
        f.write("line1\nline2\n")
        f.flush()
        try:
...
async def test_num_lines_zero_returns_error():
    with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
        f.write("line1\nline2\n")
        f.flush()
        try:
...
async def test_num_lines_negative_returns_error():
    with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
        f.write("line1\nline2\n")
        f.flush()
        try:
...
async def test_valid_start_line_still_works():
    with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
        f.write("line1\nline2\nline3\n")
        f.flush()
        try:
