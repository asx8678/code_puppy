...
class TestBaseAgentRunMCP:
    ...
    @pytest.mark.asyncio
    async def test_on_agent_exception_called_when_task_raises(self, agent):
        """Test that on_agent_exception callback fires when the agent task raises.

        The outer except-Exception block in run_with_mcp calls on_agent_exception;
        we trigger that path by making the awaited asyncio.Task raise directly
        (bypassing run_agent_task's internal except* handler).
        """
        test_error = RuntimeError("Agent task failed - on_agent_exception test")

        async def raising_coro():
            raise test_error

        # Pre-create a real task so we have a proper awaitable that raises.
        fail_task = asyncio.ensure_future(raising_coro())

        def _create_task_side_effect(coro, **kwargs):
            # Close the original coroutine to suppress "never awaited" warnings.
            coro.close()
            return fail_task

        with patch(
            "code_puppy.agents.base_agent.on_agent_exception",
            ...
            with patch(
                ...
                with pytest.raises(RuntimeError, match="Agent task failed"):
                    await agent.run_with_mcp("on_agent_exception test prompt")

            # Callback must have been invoked exactly once.
            mock_on_exc.assert_called_once()
            # First positional argument should be the original exception.
            exc_arg = mock_on_exc.call_args[0][0]
            assert exc_arg is test_error

    @pytest.mark.asyncio
    async def test_on_agent_exception_failure_does_not_mask_original_exception(
        self, agent
    ):
        """Test that a failing on_agent_exception callback doesn't mask the original error.

        The implementation wraps the callback call in try/except so that even if
        the callback itself raises, the original exception is still propagated.
        """
        original_error = ValueError("Original agent error - must propagate")
        ...
        # Callback raises a *different* exception to verify it doesn't mask the original.
        exploding_callback = AsyncMock(
            side_effect=RuntimeError("on_agent_exception callback itself exploded")
        )
        ...
        with patch(
            ...
            with patch(
                ...
                # The ORIGINAL ValueError (not the callback's RuntimeError) must surface.
                with pytest.raises(ValueError, match="Original agent error"):
                    await agent.run_with_mcp("callback failure test")

        # Confirm the callback was at least attempted.
        exploding_callback.assert_called_once()
