...
class TestProjectAgentIntegration:
    ...
    def test_discover_project_agent_via_cwd(self, tmp_path, monkeypatch):
        """Test that changing to a directory with .fast_puppy discovers agents."""
        # Create project structure: project/.fast_puppy/agents/
        project_dir = tmp_path / "myproject"
        project_dir.mkdir()
        agents_dir = project_dir / ".fast_puppy" / "agents"
    ...
    def test_no_project_agent_when_outside_project(self, tmp_path, monkeypatch):
        ...
        # Create different directory without .fast_puppy
    ...
    def test_project_agent_overrides_user_agent_via_cwd(self, tmp_path, monkeypatch):
        ...
        project_dir = tmp_path / "myproject"
        project_dir.mkdir()
        project_agents_dir = project_dir / ".fast_puppy" / "agents"
    ...
    def test_nested_project_discovery(self, tmp_path, monkeypatch):
        """Test that .fast_puppy is found from nested subdirectories."""
        # Create project/.fast_puppy/agents/
        project_dir = tmp_path / "myproject"
        project_dir.mkdir()
        agents_dir = project_dir / ".fast_puppy" / "agents"
        agents_dir.mkdir(parents=True)
        ...
        # the agent from nested directories (would need to walk up to find .fast_puppy)
