...
def mock_config_paths(monkeypatch):
    ...
    monkeypatch.setattr(
        os.path,
        "expanduser",
        lambda path: mock_home if path == "~" else _original_expanduser(path),
    )
...
class TestGetYoloMode:
    ...
    def test_get_yolo_mode_from_config_true(self, mock_get_value):
        ...
        for val in true_values:
            ...
            cp_config.get_yolo_mode.cache_clear()  # Clear cache between iterations
    ...
    def test_get_yolo_mode_not_in_config_defaults_true(self, mock_get_value):
        cp_config.get_yolo_mode.cache_clear()  # Clear cache before test
        mock_get_value.return_value = None
