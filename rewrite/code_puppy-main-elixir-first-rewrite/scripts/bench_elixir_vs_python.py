...
class BenchmarkResult:
    ...
    # Benchmark results
    spawn_latency: LatencyStats = field(default_factory=lambda: LatencyStats(0,0,0,0,0,0,0,0))
    request_latency: LatencyStats = field(default_factory=lambda: LatencyStats(0,0,0,0,0,0,0,0))
...
class JsonRpcWorker:
    ...
    def call(self, method: str, params: dict[str, Any] = None, timeout: float = 30.0) -> dict[str, Any]:
        """
...
class ElixirControlPlaneBenchmark:
    ...
    def __init__(self, mode: BenchmarkMode):
        self.mode = mode
        self.warmup_runs = QUICK_WARMUP_RUNS if mode == BenchmarkMode.QUICK else DEFAULT_WARMUP_RUNS
        self.runs = QUICK_BENCHMARK_RUNS if mode == BenchmarkMode.QUICK else DEFAULT_BENCHMARK_RUNS
    ...
    def benchmark_concurrent(self) -> list[ConcurrentResult]:
        ...
        worker_counts = [1, 2, 4, 8, 16] if self.mode == BenchmarkMode.FULL else [1, 4, 8]
...
class PythonOnlyBenchmark:
    ...
    def benchmark_fault_recovery(self) -> FaultRecoveryResult:
        ...
        new_proc = subprocess.Popen(
            [sys.executable, "-c", "import sys; print('ready', flush=True); sys.exit(0)"],
            stdout=subprocess.PIPE,
            text=True
        )
...
def format_comparison(elixir_stats: LatencyStats, python_stats: LatencyStats) -> str:
    ...
    ratio = elixir_stats.mean_ms / python_stats.mean_ms if python_stats.mean_ms > 0 else 0
...
def run_benchmarks(
    ...
    elixir_result = BenchmarkResult(
        approach="elixir_control_plane",
        timestamp=timestamp,
        mode=mode.value
    )
    python_result = BenchmarkResult(
        approach="python_only",
        timestamp=timestamp,
        mode=mode.value
    )
    ...
    if "spawn" in benchmarks_to_run:
        ...
        print("\n" + "-" * 40)
        print(format_comparison(elixir_result.spawn_latency, python_result.spawn_latency))
    ...
    if "latency" in benchmarks_to_run:
        ...
        print("\n" + "-" * 40)
        print(format_comparison(elixir_result.request_latency, python_result.request_latency))
    ...
    if "throughput" in benchmarks_to_run:
        ...
        print("   " + format_latency(elixir_result.throughput.latency_per_op_ms).replace("  ", " "))
        ...
        print("  Per-op latency:")
        print("   " + format_latency(python_result.throughput.latency_per_op_ms).replace("  ", " "))
        
        ratio = elixir_result.throughput.ops_per_sec / python_result.throughput.ops_per_sec
        slower_faster = "slower" if ratio < 1 else "faster"
        print(f"\n{'-' * 40}")
        print(f"  Elixir Control Plane is {1/ratio:.2f}x {slower_faster} than Python-Only")
    ...
    if "concurrent" in benchmarks_to_run:
        ...
        for result in elixir_result.concurrent:
            ...
            print("    Per-op latency:")
            print("     " + format_latency(result.per_worker_latency_ms).replace("  ", " "))
    ...
    if "fault" in benchmarks_to_run:
        ...
        print(f"  Detection time: {elixir_result.fault_recovery.detection_time_ms:.3f}ms")
        print(f"  Respawn time: {elixir_result.fault_recovery.respawn_time_ms:.3f}ms")
        print(f"  Total recovery: {elixir_result.fault_recovery.total_recovery_ms:.3f}ms")
        
        print("\nRunning Python-Only...")
        python_result.fault_recovery = python_bench.benchmark_fault_recovery()
        print(f"  Detection time: {python_result.fault_recovery.detection_time_ms:.3f}ms")
        print(f"  Respawn time: {python_result.fault_recovery.respawn_time_ms:.3f}ms")
        print(f"  Total recovery: {python_result.fault_recovery.total_recovery_ms:.3f}ms")
    
    return {
        "elixir_control_plane": elixir_result,
        "python_only": python_result
    }
...
def serialize_results(results: dict[str, BenchmarkResult]) -> dict:
    ...
    def asdict_recursive(obj):
        if isinstance(obj, (LatencyStats, ThroughputResult, ConcurrentResult, 
                           FaultRecoveryResult, BenchmarkResult)):
