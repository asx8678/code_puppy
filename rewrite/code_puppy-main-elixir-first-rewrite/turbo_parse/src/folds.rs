...
use crate::queries::{get_folds_query, QueryError};
...
/// Normalize language name to our supported set.
...
/// # Example
/// ```
/// use turbo_parse::folds::get_folds;
///
...
"#;
    ...
    #[test]
    fn test_get_folds_python_if_else() {
...
"#;
        ...
        assert!(result.success);
        // Should have if_statement folds
        assert!(!result.folds.is_empty());
    }

    #[test]
    fn test_get_folds_python_for_while() {
        let source = r#"for i in range(10):
    pass
...
"#;
        ...
        assert!(result.success);
        assert!(result.folds.iter().any(|f| f.fold_type == FoldType::Loop));
    }
...
"#;
        ...
        assert!(result.success);
        assert!(result.folds.iter().any(|f| {
            f.fold_type == FoldType::Conditional // match expression
        }));
    }
...
"#;
    ...
    #[test]
    fn test_get_folds_nested() {
        // Test that nested folds are properly detected and ordered
...
"#;
        ...
        assert!(result.success);
        assert!(result.count() >= 2); // At least outer and inner class

        // Folds should be ordered by start line
        for i in 1..result.folds.len() {
            assert!(
                result.folds[i].start_line >= result.folds[i - 1].start_line,
                "Folds should be ordered by start line"
            );
        }
    }
    ...
    #[test]
    fn test_get_folds_no_folds() {
        // File with no foldable structures
        let source = r#"x = 1
y = 2
print(x + y)
"#;
        ...
        assert!(result.success);
        // May have zero or minimal folds
        assert!(result.folds.len() < 3);
    }
