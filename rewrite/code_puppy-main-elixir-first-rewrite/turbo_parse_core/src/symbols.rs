...
use crate::queries::normalize_language;
use crate::registry::get_language;
use crate::types::{Symbol, SymbolOutline, RegistryError};
...
fn execute_symbol_query(
    ...
    while let Some(m) = matches.next() {
        ...
        if let (Some(kind), Some(name_node)) = (kind_capture, name_node) {
            if let Some(name) = extract_node_name(name_node, source) {
                ...
                let symbol_kind = kind.to_string();

                // Check for duplicates
                let exists = symbols.iter().any(|(s, _)| {
                    s.name == name && s.start_line == start_line && s.start_column == start_col
                });
    ...
    for (symbol, _kind_node) in symbols {
        let symbol = if symbol.kind == "function" || symbol.kind == "method" {
...
fn find_method_parent(
    ...
    let target_byte = find_byte_offset(source, symbol.start_line - 1, symbol.start_column);
...
mod tests {
    ...
    fn test_symbol_new() {
        ...
        assert_eq!(sym.start_column, 0);
        assert_eq!(sym.end_column, 5);
    ...
    fn test_symbol_outline_new() {
        let symbols = vec![
            Symbol::new("func1", "function", 1, 0, 5, 10),
            Symbol::new("Class1", "class", 7, 0, 15, 20),
        ];
    ...
    fn test_symbol_outline_filtering() {
        let symbols = vec![
            Symbol::new("func1", "function", 1, 0, 5, 10),
            Symbol::new("Class1", "class", 7, 0, 15, 20),
            Symbol::new("method1", "method", 8, 4, 10, 15).with_parent("Class1"),
        ];
        let outline = SymbolOutline::new("python", symbols, 0.5, true);
        
        let functions = outline.symbols_of_kind("function");
        assert_eq!(functions.len(), 1);
        assert_eq!(functions[0].name, "func1");
        
        let classes = outline.symbols_of_kind("class");
...
"#;
        ...
        assert!(outline.success);
        assert_eq!(outline.language, "python");
        assert!(outline.symbols.iter().any(|s| s.name == "hello" && s.kind == "function"));
    }
...
"#;
        ...
        assert!(outline.success);
        assert!(outline.symbols.iter().any(|s| s.name == "MyClass" && s.kind == "class"));
...
"#;
        ...
        assert!(outline.success);
        assert!(outline.symbols.iter().any(|s| s.name == "main" && s.kind == "function"));
        assert!(outline.symbols.iter().any(|s| s.name == "Point" && s.kind == "struct"));
    }
...
"#;
        ...
        assert!(outline.success);
        assert!(outline.symbols.iter().any(|s| s.name == "greet" && s.kind == "function"));
        assert!(outline.symbols.iter().any(|s| s.name == "MyClass" && s.kind == "class"));
    }
...
"#;
        ...
        assert!(outline.success);
        assert!(outline.symbols.iter().any(|s| s.name == "User" && s.kind == "interface"));
        assert!(outline.symbols.iter().any(|s| s.name == "getUser" && s.kind == "function"));
    }
...
"#;
        ...
        assert!(outline.success);
        assert!(outline.symbols.iter().any(|s| s.name == "MyModule" && s.kind == "module"));
        assert!(outline.symbols.iter().any(|s| s.name == "hello" && s.kind == "function"));
    }
...
"#;
        ...
        assert!(outline.success);
        let imports = outline.symbols_of_kind("import");
