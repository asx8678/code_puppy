...
async def find_by_role(
    role: str, name: str | None = None, exact: bool = False, timeout: int = 10000
) -> dict[str, Any]:
    """Find elements by ARIA role."""
    group_id = generate_group_id("browser_find_by_role", f"{role}_{name or 'any'}")
    emit_info(
        f"BROWSER FIND BY ROLE 🎨 role={role} name={name}", message_group=group_id
    )
    try:
        ...
        emit_success(
            f"Found {count} elements with role '{role}'", message_group=group_id
        )
...
async def find_by_text(
    ...
    emit_info(
        f"BROWSER FIND BY TEXT 🔍 text='{text}' exact={exact}", message_group=group_id
    )
    try:
        browser_manager = get_session_browser_manager()
        page = await browser_manager.get_current_page()
        ...
        emit_success(
            f"Found {count} elements containing text '{text}'", message_group=group_id
        )
...
async def find_by_label(
    ...
    emit_info(
        f"BROWSER FIND BY LABEL 🏷️ label='{text}' exact={exact}", message_group=group_id
    )
    try:
        browser_manager = get_session_browser_manager()
        page = await browser_manager.get_current_page()
        ...
        emit_success(
            f"Found {count} elements with label '{text}'", message_group=group_id
        )
...
async def find_by_placeholder(
    ...
    try:
        ...
        emit_success(
            f"Found {count} elements with placeholder '{text}'", message_group=group_id
        )
...
async def find_by_test_id(test_id: str, timeout: int = 10000) -> dict[str, Any]:
    """Find elements by test ID attribute."""
    group_id = generate_group_id("browser_find_by_test_id", test_id)
    emit_info(f"BROWSER FIND BY TEST ID 🧪 test_id='{test_id}'", message_group=group_id)
    try:
        browser_manager = get_session_browser_manager()
        page = await browser_manager.get_current_page()
        ...
        emit_success(
            f"Found {count} elements with test-id '{test_id}'", message_group=group_id
        )
...
async def run_xpath_query(xpath: str, timeout: int = 10000) -> dict[str, Any]:
    """Find elements using XPath selector."""
    group_id = generate_group_id("browser_xpath_query", xpath[:100])
    emit_info(f"BROWSER XPATH QUERY 🔍 xpath='{xpath}'", message_group=group_id)
    try:
        browser_manager = get_session_browser_manager()
        page = await browser_manager.get_current_page()
        ...
        emit_success(
            f"Found {count} elements with XPath '{xpath}'", message_group=group_id
        )
...
def register_find_by_text(agent):
    ...
    async def browser_find_by_text(
        context: RunContext, text: str, exact: bool = False, timeout: int = 10000
    ) -> dict[str, Any]:
        """
...
def register_find_by_test_id(agent):
    ...
    async def browser_find_by_test_id(
        context: RunContext, test_id: str, timeout: int = 10000
    ) -> dict[str, Any]:
        """
...
def register_run_xpath_query(agent):
    ...
    async def browser_xpath_query(
        context: RunContext, xpath: str, timeout: int = 10000
    ) -> dict[str, Any]:
        """
...
def register_find_buttons(agent):
    ...
    async def browser_find_buttons(
        context: RunContext, text_filter: str | None = None, timeout: int = 10000
    ) -> dict[str, Any]:
        """
        Find all button elements on the page.
