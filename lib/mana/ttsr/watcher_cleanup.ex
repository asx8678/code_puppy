defmodule Mana.TTSR.WatcherCleanup do
      ...
      |> Enum.filter(fn {session_id, pid} ->
        case Process.alive?(pid) do
          true ->
            ...
            is_stale?(last_activity, now, threshold_seconds)
  ...
  defp is_stale?(nil, _now, _threshold) do
    # No activity recorded, consider stale
    true
  end

  defp is_stale?(last_activity, now, threshold_seconds) do
  ...
  defp gather_stats(threshold_seconds) do
    ...
    {active, stale} =
      Enum.reduce(entries, {0, 0}, fn {session_id, pid}, {active_acc, stale_acc} ->
        case Process.alive?(pid) do
          true ->
            last_activity = StreamWatcher.get_last_activity(session_id)

            if is_stale?(last_activity, now, threshold_seconds) do
