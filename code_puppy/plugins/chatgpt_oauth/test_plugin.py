...
def test_config_paths():
    ...
    # XDG paths use "fast_puppy" (without dot) in ~/.local/share or ~/.config
    assert "fast_puppy" in str(token_path)

    config_dir = config.get_config_dir()
    # Default is ~/.fast_puppy; XDG paths only used when XDG env vars are set
    assert config_dir.name in ("fast_puppy", ".fast_puppy")
