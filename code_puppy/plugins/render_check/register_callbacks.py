...
def _test_plain_and_styled(console) -> None:
    ...
    console.print("[bold red on white] alert [/bold red on white] [bold green on black] ok [/bold green on black]")
...
def _test_panels(console) -> None:
    ...
    console.print(Panel.fit("[bold yellow]Warning![/bold yellow] Fit panel with rich content.", border_style="yellow", title="Warning"))
    console.print(Panel("[green]Success[/green] — operation completed.", border_style="green", title="Success", subtitle="[dim]subtitle[/dim]"))
...
def _test_environment(console) -> None:
    ...
    t.add_row("CODE_PUPPY_FORCE_COLOR", os.environ.get("CODE_PUPPY_FORCE_COLOR", "<unset>"))
...
def _run_render_check() -> None:
    ...
    console.print("[dim]If anything below looks wrong, your terminal may not support certain Rich features.[/dim]")
    try:
        ...
        console.print("[dim]Tip: set CODE_PUPPY_FORCE_COLOR=1 to force colors when piping, or CODE_PUPPY_NO_COLOR=1 to disable.[/dim]")
        console.print()
    except Exception as e:
...
def _on_custom_command_help():
    return [(f"/{COMMAND_NAME}", "Run a visual test pattern to verify terminal rendering")]
