"""Status line plugin — a customizable bottom status line for Fast Puppy.

Mirrors Claude Code's ``statusLine`` feature: you configure a shell command;
Fast Puppy feeds it JSON session data on stdin; whatever the command prints to
...
Configure with the ``/statusline`` command or directly via ``/set``:
    ...
    /set statusline_command=~/.fast_puppy/statusline.sh
