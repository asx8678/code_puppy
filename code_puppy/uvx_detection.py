"""Detect if fast-puppy was launched via uvx on Windows.

This module provides utilities to detect the launch method of fast-puppy,
specifically to handle signal differences when running via uvx on Windows.

On Windows, when launched via `uvx fast-puppy`, Ctrl+C (SIGINT) gets captured
...
def is_launched_via_uvx() -> bool:
    """Detect if fast-puppy was launched via uvx.
