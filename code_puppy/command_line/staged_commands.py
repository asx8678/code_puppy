...
def handle_staged_command(command: str) -> bool:
    ...
    if subcommand == "off":
        sandbox.disable()
        emit_warning("📝 Staging mode disabled - file edits will be applied immediately")
        return True
    ...
    # Invalid usage
    emit_warning("Usage: /staged [on|off|diff|preview|clear|apply|reject|save|load|status]")
    return True
