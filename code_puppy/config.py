...
def get_config_keys():
    ...
    # Add consensus planner configuration keys
    default_keys.append("consensus_planner_enabled")
    default_keys.append("consensus_planner_threshold")
    default_keys.append("consensus_planner_always_on")
    default_keys.append("preferred_consensus_models")
    default_keys.append("consensus_planner_swarm_size")
    default_keys.append("consensus_planner_timeout")
...
def get_consensus_planner_enabled() -> bool:
    """Check if the consensus planner is enabled.

    Returns:
        True if consensus planner is enabled, False otherwise.
    """
    return _is_truthy(get_value("consensus_planner_enabled"), default=True)


def get_consensus_planner_threshold() -> float:
    """Get the complexity threshold for triggering consensus.

    Tasks with complexity scores above this threshold will use
    multi-model consensus instead of single model execution.

    Returns:
        Float between 0.0 and 1.0 (default: 0.7)
    """
    val = get_value("consensus_planner_threshold")
    if val:
        try:
...
def get_consensus_planner_always_on() -> bool:
    """Check if /consensus:always mode is enabled.

    When enabled, all tasks will use multi-model consensus regardless
    of complexity analysis.

    Returns:
        True if always-on mode is enabled, False otherwise.
    """
    return _is_truthy(get_value("consensus_planner_always_on"), default=False)


def get_preferred_consensus_models() -> list[str]:
    """Get the list of preferred models for consensus operations.
...
def get_consensus_planner_swarm_size() -> int:
    """Get the default swarm size for consensus planner.

    Returns:
        Number of agents to spawn (default: 3, range: 2-5)
    """
    val = get_value("consensus_planner_swarm_size")
    if val:
        try:
            return max(2, min(5, int(val)))
        except ValueError:
            pass
    return 3


def get_consensus_planner_timeout() -> int:
    """Get the timeout in seconds for consensus operations.

    Returns:
        Timeout in seconds (default: 180, range: 30-600)
    """
    val = get_value("consensus_planner_timeout")
    if val:
        try:
            return max(30, min(600, int(val)))
        except ValueError:
            pass
    return 180
...
# =============================================================================
# Consensus Auto-Spawn Configuration
# =============================================================================
...
def get_consensus_auto_spawn_enabled() -> bool:
    ...
    When enabled, the system will automatically suggest or trigger
    ConsensusPlanner when uncertainty, errors, or complexity are detected.

    Returns:
        True if auto-spawn is enabled, False otherwise.
    """
    return _is_truthy(get_value("consensus_auto_spawn_enabled"), default=True)


def get_consensus_auto_spawn_triggers() -> list[str]:
    """Get the list of enabled auto-spawn triggers.

    Returns:
        List of trigger types: uncertainty, error, complexity, self_correction
    """
    val = get_value("consensus_auto_spawn_triggers")
    if val:
        return [t.strip() for t in val.split(",") if t.strip()]
    return ["uncertainty", "error", "complexity"]


def get_consensus_uncertainty_threshold() -> float:
    """Get the uncertainty threshold for auto-spawning consensus.

    When confidence score falls below this threshold, consensus is triggered.

    Returns:
        Float between 0.0 and 1.0 (default: 0.6)
    """
    val = get_value("consensus_uncertainty_threshold")
    if val:
        try:
            return max(0.0, min(1.0, float(val)))
        except ValueError:
...
def get_consensus_ask_before_spawn() -> bool:
    """Check if the system should ask before auto-spawning consensus.

    When True, the system will ask user confirmation before spawning.
    When False, consensus is spawned automatically when triggered.

    Returns:
        True if should ask before spawning, False otherwise.
    """
    return _is_truthy(get_value("consensus_ask_before_spawn"), default=True)
